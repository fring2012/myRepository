...
public class MqttService extends Service implements MqttTraceHandler {
    ...
    public AlarmManager startKeepConnect(long repeatTime, long triggerTime) {
        Trace.d(TAG, "startKeepConnect() repeat time:" + ((repeatTime + 0f) / 1000 / 60 / 60) + "h,next time:" + getFormateTime(triggerTime));
    ...
    private class KeepConnectReceiver extends BroadcastReceiver {

        public KeepConnectReceiver() {
        }
        ...
        public void onReceive(Context context, Intent intent) {
            switch (intent.getAction()) {
                case ACTION_ALARM_KEEP_CONNECT:
                    ...
                    if (Utils.isNetWorkAvailable(MqttService.this) && Utils.isAvaliableByPing()) {
                        Trace.d(TAG, "onReceive() try to reconnect" + Utils.isNetWorkAvailable(MqttService.this) + "--------------" + Utils.isAvaliableByPing());
                        reconnect();
                    } else {
    ...
    public boolean containClient(String clientHandle) {
